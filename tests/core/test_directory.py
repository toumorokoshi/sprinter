...
class TestDirectory(object):
    ...
    def setup(self):
        ...
        self.directory = Directory(
            os.path.join(self.temp_dir, "test"), rewrite_config=True
        )
    ...
    def test_symlink_to_bin(self):
        ...
        try:
            ...
            tools.eq_(
                open(os.path.join(self.directory.bin_path(), "newfile")).read(),
                open(temp_file_path).read(),
                "File contents are different for symlinked files!",
            )
            assert os.access(
                os.path.join(self.directory.bin_path(), "newfile"), os.X_OK
    ...
    def test_symlink_to_include(self):
        ...
        tools.eq_(
            open(os.path.join(self.directory.include_path(), "newfile")).read(),
            open(temp_file).read(),
            "File contents are different for symlinked files!",
        )
    ...
    def test_add_to_rc(self):
        ...
        del self.directory
        assert (
            open(rc_file_path).read().find(test_content) != -1
    ...
    def test_remove(self):
        ...
        ), "Path still exists after remove!"
