...
class SSHFormula(FormulaBase):
    ...
    def install(self):
        ...
        if self.target.has("install_command") and not self.target.is_affirmative(
            "use_global_ssh", default="no"
        ):
            self.__call_command(
                self.target.get("install_command"), self.target.get("ssh_key_path")
            )
    ...
    def __generate_key(self, config):
        ...
        if config.is_affirmative("create", default="yes"):
            ...
            if not os.path.exists(os.path.join(cwd, config.get("keyname"))):
                command = (
                    "ssh-keygen -t %(type)s -f %(keyname)s -N  " % config.to_dict()
                )
                lib.call(command, cwd=cwd, output_log_level=logging.DEBUG)
        if not config.has("ssh_path"):
            config.set("ssh_path", cwd)
        config.set(
            "ssh_key_path", os.path.join(config.get("ssh_path"), config.get("keyname"))
        )
        ...
        if not config.is_affirmative("use_global_ssh", default="no"):
            ...
            if not os.path.exists(ssh_config_path):
                ...
                if self.injections.in_noninjected_file(
                    ssh_config_path, "Host %s" % config.get("host")
                ):
    ...
    def __call_command(self, command, ssh_path):
        ...
        lib.call(
            command, shell=True, output_log_level=logging.DEBUG, sensitive_info=True
        )
