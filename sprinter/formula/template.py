...
class TemplateFormula(FormulaBase):
    ...
    def remove(self):
        if self.source.is_affirmative("remove_file_on_delete", False):
            os.path.unlink(os.path.expanduser(self.source.get("target")))
        FormulaBase.remove(self)

    def validate(self):
        if self.target:
            if (
                self.target.has("username")
                and not self.target.has("password")
        ...
        if source.startswith("http"):
            if config.has("username") and config.has("password"):
                source_content = self.lib.authenticated_get(
                    config.get("username"), config.get("password"), source
                ).decode("utf-8")
            else:
