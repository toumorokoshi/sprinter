...
from sprinter.formula.git import (
    CURRENT_REMOTE,
    CURRENT_BRANCH,
    CLONE_REPO,
    CHECKOUT_BRANCH,
...
vals = {"repoA": "git://github.com/toumorokoshi/sprinter.git"}
...
class TestGitFormula(FormulaTest):
    ...
    def setup(self):
        super(TestGitFormula, self).setup(
    ...
    def test_simple_example(self, call_mock):
        ...
        call_mock.assert_has_calls(
            [
                call(
                    CLONE_REPO.format(repo=vals["repoA"], dir=install_directory),
    ...
    def test_update_different_branches(self, call_mock):
        ...
        call_count = 0
        return_values = {
            CURRENT_REMOTE.format(
                dir=install_directory
            ): "git://github.com/toumorokoshi/sprinter.git",
            CURRENT_BRANCH.format(dir=install_directory): "master",
        }
        call_mock.side_effect = lambda cmd, **kw: (
            0,
            return_values[cmd] if cmd in return_values else "",
        )
    ...
    def test_update_no_directory(self, call_mock):
        ...
        call_mock.assert_any_call(
            CLONE_REPO.format(
                repo=vals["repoA"], dir=self.directory.install_directory("update")
            ),
            output_log_level=logging.DEBUG,
        )
