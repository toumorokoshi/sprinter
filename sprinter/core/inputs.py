...
class Inputs(object):
    ...
    def get_input(self, key, force=False):
        ...
        help_text = (
            self._inputs[key].help if hasattr(self._inputs[key], "help") else None
        )
    ...
    def values(self, with_defaults=True):
        ...
        return dict(
            (
                (k, str(v))
                for k, v in self._inputs.items()
                if not v.is_empty(with_defaults)
            )
        )
    ...
    def write_values(self):
        ...
        return dict(
            (
                (k, v.value)
                for k, v in self._inputs.items()
                if not v.is_secret and not v.is_empty(False)
            )
        )
    ...
    def add_inputs_from_inputstring(self, input_string):
        ...
        param_attributes = (
            self._parse_param_line(rp) for rp in raw_params if len(rp.strip(" \t")) > 0
        )
            ...
            if value.find("#") != -1:
                ...
                try:
                    ...
                    raise InputException(
                        "Incorrectly formatted input for {0}!".format(value)
                    )
