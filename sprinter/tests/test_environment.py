...
from sprinter.testtools import MockEnvironment, create_mock_formulabase
...
class TestEnvironment(object):
    ...
    def test_grab_inputs_existing_source(self):
        ...
        with MockEnvironment(source_config, target_config) as environment:
            ...
            eq_(
                environment.target.inputs.get_unset_inputs(),
                set(["password", "main_branch"]),
            )
    ...
    def test_feature_run_order_install(self, mock_feature):
        ...
        with patch(
            "sprinter.core.featuredict.Feature", new=lambda *args: mock_feature
        ) as mock_call:
            with MockEnvironment(test_source, test_target) as environment:
                environment.install()
                eq_(
                    mock_call().method_calls,
                    [
                        call.should_run(),
                        call.validate(),
                        call.resolve(),
    ...
    def test_global_shell_configuration_bash(self):
        ...
        with MockEnvironment(
            test_source, test_target, global_config=global_config
        ) as environment:
            environment.install()
            assert [
                x
                for x in environment.injections.inject_dict.keys()
                if x.endswith(".bashrc")
            ]
            env_injected = False
            for profile in [".bash_profile", ".bash_login", ".profile"]:
                env_injected = env_injected or filter(
                    lambda x: x.endswith(profile),
                    environment.injections.inject_dict.keys(),
                )
            assert env_injected
            assert not [
                x
    ...
    def test_env_to_rc_injection(self):
        ...
        with MockEnvironment(
            ...
            for profile in [".bash_profile", ".bash_login", ".profile"]:
                ...
                specific_env_injected = (
                    full_profile_path in environment.global_injections.inject_dict
                )
                if specific_env_injected:
                    env_injected = True
                    assert (
                        source_template % (full_rc_path, full_rc_path)
                        in environment.global_injections.inject_dict[full_profile_path]
                    )
            assert env_injected
    ...
    def test_global_shell_configuration_zshell(self):
        ...
        with MockEnvironment(
            target_config=test_target, global_config=global_config
        ) as environment:
            environment.install()
...
env_source_rc = False
    ...
    def test_errors_fail_out_immediately(self):
        ...
        with patch(
            "sprinter.formula.base.FormulaBase", new=create_mock_formulabase()
        ) as formulabase:
            formulabase.install.side_effect = Exception
            with MockEnvironment(
                None, test_target, mock_formulabase=formulabase
            ) as environment:
    ...
    def test_feature_run_remove_failure(self):
        ...
        with patch(
            ...
            with MockEnvironment(
                test_source, test_target, mock_formulabase=formulabase
            ) as environment:
                environment.directory = Mock(spec=environment.directory)
                environment.directory.new = False
                environment.remove()
